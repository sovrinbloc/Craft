...
#define BLOCK_GROUND 2
#define BLOCK_STEEL 3
#define BLOCK_BRICK 4
#define BLOCK_WOOD 5
#define BLOCK_ROCK 6
#define BLOCK_DIRT 7
#define BLOCK_PLANK 8
#define BLOCK_SNOW 9
...
void make_world(Map *map, int x_chunk_pos, int z_chunk_pos) {
    ...
    for (int dx = -pad; dx < CHUNK_SIZE + pad; dx++) { // from -1 to +33?
        for (int dz = -pad; dz < CHUNK_SIZE + pad; dz++) {
            ...
            float f = simplex2(x * 0.01, z * 0.01, 4, 0.5, 2); // increases depth (sheer)
            ...
            // whatis: why would we had the padding, then?
            if (dx < 0 || dz < 0 || dx >= CHUNK_SIZE || dz >= CHUNK_SIZE) {
                block_texture = -1; // empty sky that is untouchable and you can't go through it
            ...
            for (int y = 0; y < block_y; y++) {
                ...
                map_set(map, x, y, z, block_texture);
            }
            if (block_texture == Grass) {
                if (simplex2(-x * 0.1, z * 0.1, 4, 0.8, 2) > 0.6) {
                    map_set(map, x, block_y, z, Weeds);
                }
                if (simplex2(x * 0.05, -z * 0.05, 4, 0.8, 2) > 0.7) {
                    int w = PlantYellow + simplex2(x * 0.1, z * 0.1, 4, 0.8, 2) * 7;
                    map_set(map, x, block_y, z, w);
                }
            }
            // render clouds between 64 and 72 height
            for (int y = 62; y < 72; y++) {
                if (simplex3(x * 0.01, y * 0.1, z * 0.01, 8, 0.5, 2) > 0.75) {
                    map_set(map, x, y, z, Clouds); // clouds
                }
            }
        }
    }
...
void make_chunk(Chunk *chunk, int x_chunk_pos, int z_chunk_pos) {
    ...
    chunk->q = z_chunk_pos;
    chunk->faces = 0;
    Map *map = &chunk->map; // each chunk has a map
    map_alloc(map); // initialize empty map
